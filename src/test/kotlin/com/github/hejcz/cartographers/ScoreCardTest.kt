package com.github.hejcz.cartographers

import org.junit.jupiter.api.Assertions
import org.junit.jupiter.api.Test

class ScoreCardTest {
    @Test
    internal fun `counting diameters works`() {
        val points = TradingRoad40.evaluate(
            Board.create(
                """
[C][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][C][ ][ ][ ]
[C][ ][M][ ][ ][ ][ ][ ][C][ ][ ]
[ ][C][ ][ ][ ][ ][ ][M][ ][C][ ]
[C][ ][C][ ][ ][ ][ ][ ][ ][ ][C]
"""
            )
        )
        Assertions.assertEquals(3 * 3, points)
    }

    @Test
    internal fun `counting diameters works 2`() {
        val points = TradingRoad40.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[C][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[C][C][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][C][ ][ ][ ][ ][ ][ ][ ]
[C][ ][ ][C][C][ ][ ][M][ ][ ][ ]
[C][C][ ][ ][C][C][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 3, points)
    }

    @Test
    internal fun `diameters in different directions dont work`() {
        val points = TradingRoad40.evaluate(
            Board.create(
                """
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
[C][ ][ ][M][ ][ ][ ][ ][ ][ ][C]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][C]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][C][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `diamatere with missing field`() {
        val points = TradingRoad40.evaluate(
            Board.create(
                """
[C][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[C][ ][ ][ ][ ][ ][ ][C][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][C][ ][ ]
[C][ ][C][ ][ ][ ][ ][M][ ][C][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `borderlands handle columns`() {
        val points = Borderlands38.evaluate(
            Board.create(
                """
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][M][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][M][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][M][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][M][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][M][ ][ ][ ]
[ ][C][ ][ ][C][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 6, points)
    }

    @Test
    internal fun `borderlands accept mountains`() {
        val points = Borderlands38.evaluate(
            Board.create(
                """
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][M][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][M][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][M][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][M][ ][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][M][ ][ ][ ]
[ ][C][C][ ][C][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 6, points)
    }

    @Test
    internal fun `borderlands handle rows`() {
        val points = Borderlands38.evaluate(
            Board.create(
                """
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
[ ][C][ ][M][ ][ ][ ][ ][ ][C][ ]
[ ][C][ ][ ][ ][ ][ ][ ][M][C][ ]
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
[ ][C][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[C][C][C][C][C][M][C][C][C][C][C]
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
[ ][C][M][ ][ ][ ][ ][ ][ ][C][ ]
[ ][C][ ][ ][ ][ ][ ][M][ ][C][ ]
[ ][C][ ][ ][ ][ ][ ][ ][ ][C][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 6, points)
    }

    @Test
    internal fun `hideouts work`() {
        val points = Hideouts41.evaluate(
            Board.create(
                """
[ ][ ][W][ ][W][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][P][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][P][ ][M][ ][ ]
[ ][ ][ ][W][ ][ ][ ][P][ ][ ][ ]
[ ][ ][W][ ][W][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][W][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 1, points)
    }

    @Test
    internal fun `hideouts work 2`() {
        val points = Hideouts41.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][C][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][C][C][C]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][C][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][C]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][C][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1, points)
    }

    @Test
    internal fun `biggest square works`() {
        val points = LostDemesne39.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[C][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 3, points)
    }

    @Test
    internal fun `biggest square works 2`() {
        val points = LostDemesne39.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 3, points)
    }

    @Test
    internal fun `biggest square works 3`() {
        val points = LostDemesne39.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][C][C][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][C][ ][ ][M][ ][ ][ ]
[ ][ ][C][C][C][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 3, points)
    }

    @Test
    internal fun `biggest square works 4`() {
        val points = LostDemesne39.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][C][C][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][ ][ ][ ][M][ ][ ][ ]
[ ][ ][C][C][C][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 3, points)
    }

    @Test
    internal fun `biggest square works 5`() {
        val points = LostDemesne39.evaluate(
            Board.create(
                """
[C][C][C][C][C][C][C][C][C][C][C]
[C][C][C][M][C][C][C][C][C][C][C]
[C][C][C][C][C][C][C][C][M][C][C]
[C][C][C][C][C][C][C][C][C][C][C]
[C][C][C][C][C][C][C][C][C][C][C]
[C][C][C][C][C][M][C][C][C][C][C]
[C][C][C][C][C][C][C][C][C][C][C]
[C][C][C][C][C][C][C][C][C][C][C]
[C][C][M][C][C][C][C][C][C][C][C]
[C][C][C][C][C][C][C][M][C][C][C]
[C][C][C][C][C][C][C][C][C][C][C]
"""
            )
        )
        Assertions.assertEquals(11 * 3, points)
    }

    @Test
    internal fun `forest tower 1`() {
        val points = ForestTower28.evaluate(
            Board.create(
                """
[F][ ][F][M][ ][ ][ ][ ][ ][ ][ ]
[F][F][F][C][ ][ ][ ][ ][M][ ][ ]
[M][C][C][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 1, points)
    }

    @Test
    internal fun `forest tower 2`() {
        val points = ForestTower28.evaluate(
            Board.create(
                """
[F][F][F][F][ ][ ][ ][ ][ ][ ][ ]
[F][F][F][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][F][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 1, points)
    }

    @Test
    internal fun `huge city 1`() {
        val points = HugeCity35.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][C][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][C][C][C][ ][ ][ ]
[ ][ ][M][ ][ ][C][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(5 * 1, points)
    }

    @Test
    internal fun `huge city 2`() {
        val points = HugeCity35.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][C][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][C][C][ ][ ][ ]
[ ][ ][M][ ][C][C][C][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(9 * 1, points)
    }

    @Test
    internal fun `huge city 3 - cant be near mountain`() {
        val points = HugeCity35.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][C][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][C][C][ ][ ][ ]
[ ][ ][M][ ][C][C][C][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][C][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 1, points)
    }

    @Test
    internal fun `huge city 4 - cant be near mountain`() {
        val points = HugeCity35.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][C][C][ ][ ][ ]
[ ][ ][ ][ ][ ][M][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][C][C][ ][ ][ ]
[ ][ ][M][ ][C][C][C][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][C][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 1, points)
    }

    @Test
    internal fun `golden breadbasket 1`() {
        val points = GoldenBreadbasket32.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][W][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][P][W][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 1 + 2 * 1, points)
    }

    @Test
    internal fun `golden breadbasket 2`() {
        val points = GoldenBreadbasket32.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][W][ ][M][ ][P][ ][ ][ ][ ][ ]
[ ][P][W][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 2 + 2 * 1, points)
    }

    @Test
    internal fun `golden breadbasket 3`() {
        val points = GoldenBreadbasket32.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][W][ ][M][W][P][W][ ][ ][ ][ ]
[ ][P][W][ ][ ][W][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 2 + 6 * 1, points)
    }

    @Test
    internal fun `golden breadbasket 4`() {
        val points = GoldenBreadbasket32.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][W][ ][M][W][P][W][ ][ ][ ][ ]
[ ][P][W][ ][ ][W][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][W][P][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 2 + 6 * 1, points)
    }

    @Test
    internal fun `mountain woods 1`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `mountain woods 2`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][F][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 3, points)
    }

    @Test
    internal fun `mountain woods 3`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][F][M][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 3, points)
    }

    @Test
    internal fun `mountain woods 4`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][F][F][M][ ][F][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][M][F][F][F][ ][F][ ][ ][ ]
[ ][ ][ ][ ][ ][F][F][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][F][F][F][ ][ ]
"""
            )
        )
        Assertions.assertEquals(5 * 3, points)
    }

    @Test
    internal fun `mountain woods 5`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][F][F][F][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][F][F][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][F][F][M][ ][F][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][F][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][F][ ][ ][ ]
[ ][ ][M][F][F][F][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][F][F][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][F][F][F][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 3, points)
    }

    @Test
    internal fun `mountain woods 6`() {
        val points = MountainWoods29.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][P][P][P][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][P][P][M][ ][ ]
[ ][ ][ ][P][ ][ ][ ][P][ ][ ][ ]
[ ][ ][ ][P][ ][ ][ ][P][ ][ ][ ]
[ ][ ][ ][P][P][M][ ][P][ ][ ][ ]
[ ][ ][ ][P][ ][ ][ ][P][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][P][ ][ ][ ]
[ ][ ][M][P][P][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][P][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][P][P][P][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `colony 1`() {
        val points = Colony34.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 8, points)
    }

    @Test
    internal fun `colony 2`() {
        val points = Colony34.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 8, points)
    }

    @Test
    internal fun `colony 3`() {
        val points = Colony34.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][C][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 8, points)
    }

    @Test
    internal fun `colony 4`() {
        val points = Colony34.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][C][C][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][ ][ ][M][ ][ ]
[ ][ ][ ][ ][C][C][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 8, points)
    }

    @Test
    internal fun `colony 5`() {
        val points = Colony34.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][C][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][C][C][ ][ ][M][ ][ ]
[ ][ ][ ][ ][C][C][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][C][C][C][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 8, points)
    }

    @Test
    internal fun `field puddle 1`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 1 + 0 * 1, points)
    }

    @Test
    internal fun `field puddle 2`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 1 * 1, points)
    }

    @Test
    internal fun `field puddle 3`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 2 * 1, points)
    }

    @Test
    internal fun `field puddle 4`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][P][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 2 * 1, points)
    }

    @Test
    internal fun `field puddle 5`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 1 * 1, points)
    }

    @Test
    internal fun `field puddle 6`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 1 * 1, points)
    }

    @Test
    internal fun `field puddle 7`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][P][W][W][W][ ][ ][ ][ ]
[ ][ ][ ][P][W][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 1 + 4 * 1, points)
    }

    @Test
    internal fun `field puddle 8`() {
        val points = FieldPuddle30.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][P][W][W][W][ ][ ][ ][ ]
[ ][ ][ ][P][W][P][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][W][ ][ ][ ][ ]
[ ][ ][ ][ ][W][ ][P][W][ ][ ][ ]
[ ][ ][M][ ][ ][ ][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 1 + 7 * 1, points)
    }

    @Test
    internal fun `coppice 1`() {
        val points = Coppice27.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 1 * 1, points)
    }

    @Test
    internal fun `coppice 2`() {
        val points = Coppice27.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][F][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 1 + 1 * 2, points)
    }

    @Test
    internal fun `coppice 3`() {
        val points = Coppice27.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][F][F][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 2 + 1 * 2, points)
    }

    @Test
    internal fun `coppice 4`() {
        val points = Coppice27.evaluate(
            Board.create(
                """
[F][F][F][F][F][F][F][F][F][F][F]
[F][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[F][ ][ ][F][F][ ][ ][ ][ ][ ][ ]
[F][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 11 + 1 * 11, points)
    }

    @Test
    internal fun `coppice 5`() {
        val points = Coppice27.evaluate(
            Board.create(
                """
[ ][F][F][F][F][F][F][F][F][F][ ]
[F][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[F][ ][ ][F][F][ ][ ][ ][ ][ ][ ]
[F][ ][ ][F][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[F][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 10 + 1 * 10, points)
    }

    @Test
    internal fun `fertile plain 1`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][C][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 3, points)
    }

    @Test
    internal fun `fertile plain 2`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][C][D][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1 * 3, points)
    }

    @Test
    internal fun `fertile plain 3`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][C][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `fertile plain 4`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][C][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0 * 3, points)
    }

    @Test
    internal fun `fertile plain 5`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][D][ ][ ][ ][D][ ][ ][ ][ ][ ]
[ ][C][C][ ][W][C][ ][ ][ ][ ][ ]
[P][C][C][ ][ ][M][ ][ ][ ][ ][ ]
[ ][D][W][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][D][C][C][C][P][ ][ ][ ]
[ ][ ][M][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 3, points)
    }

    @Test
    internal fun `fertile plain 6`() {
        val points = FertilePlain36.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][D][ ][ ][ ][D][ ][ ][ ][ ][ ]
[ ][C][C][ ][W][C][ ][ ][ ][ ][ ]
[P][C][C][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][D][C][C][C][P][ ][ ][ ]
[ ][ ][M][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 3, points)
    }

    @Test
    internal fun `mages valley 1`() {
        val points = MagesValley31.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][P][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][M][P][ ][ ][ ][ ]
[ ][ ][ ][ ][W][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2 * 2 + 2 * 1, points)
    }

    @Test
    internal fun `mages valley 2`() {
        val points = MagesValley31.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][M][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 2, points)
    }

    @Test
    internal fun `mages valley 3`() {
        val points = MagesValley31.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][P][M][P][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][P][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(4 * 1, points)
    }

    @Test
    internal fun `mages valley 4`() {
        val points = MagesValley31.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][P][ ][P][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][P][ ][P][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0, points)
    }

    @Test
    internal fun `mages valley 5`() {
        val points = MagesValley31.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][P][ ][ ]
[ ][ ][ ][ ][P][P][P][ ][ ][ ][ ]
[ ][ ][ ][ ][D][M][P][ ][P][W][ ]
[ ][ ][ ][ ][C][C][P][ ][P][W][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][W][W][ ]
[ ][ ][M][W][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][W][W][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][W][ ][ ][W][W][W][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3 * 1 + 3 * 2, points)
    }

    @Test
    internal fun `forest guard 1`() {
        val points = ForestGuard26.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][F][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1, points)
    }

    @Test
    internal fun `forest guard 2`() {
        val points = ForestGuard26.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][F][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][F][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(1, points)
    }

    @Test
    internal fun `forest guard 3`() {
        val points = ForestGuard26.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][F][F][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2, points)
    }

    @Test
    internal fun `forest guard 4`() {
        val points = ForestGuard26.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][F][F][ ][ ][ ][ ]
[F][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][F]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[F][ ][ ][ ][ ][F][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(6, points)
    }

    @Test
    internal fun `forest guard 5`() {
        val points = ForestGuard26.evaluate(
            Board.create(
                """
[F][F][F][F][F][F][F][F][F][F][F]
[F][ ][ ][M][ ][ ][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][ ][ ][ ][M][ ][F]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][M][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][ ][ ][ ][ ][ ][F]
[F][ ][M][ ][ ][ ][ ][ ][ ][ ][F]
[F][ ][ ][ ][ ][ ][ ][M][ ][ ][F]
[F][F][F][F][F][F][F][F][F][F][F]
"""
            )
        )
        Assertions.assertEquals(40, points)
    }

    @Test
    internal fun `vast enbankment 1`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][P][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3, points)
    }

    @Test
    internal fun `vast enbankment 2`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3, points)
    }

    @Test
    internal fun `vast enbankment 3`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][W][W][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(3, points)
    }

    @Test
    internal fun `vast enbankment 4`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][C][C][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][D][ ][F][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0, points)
    }

    @Test
    internal fun `vast enbankment 5`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][P][W][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0, points)
    }

    @Test
    internal fun `vast enbankment 6`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][P][ ][W][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(6, points)
    }

    @Test
    internal fun `vast enbankment 7`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][W][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][P][ ][W][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][W][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(15, points)
    }

    @Test
    internal fun `vast enbankment 8`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][W][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][W][W][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][P][ ][W][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][W][ ]
[ ][ ][ ][ ][W][W][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(15, points)
    }

    @Test
    internal fun `vast enbankment 9`() {
        val points = VastEnbankment33.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][W][W][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[W][W][W][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][P][ ]
[ ][ ][ ][ ][ ][ ][ ][P][P][W][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][W][ ]
[ ][ ][ ][ ][W][W][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][W][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0, points)
    }

    @Test
    internal fun `fortress 1`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2, points)
    }

    @Test
    internal fun `fortress 2`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][C][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(2, points)
    }

    @Test
    internal fun `fortress 3`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][C][C][C][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(8, points)
    }

    @Test
    internal fun `fortress 4`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][C][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][C][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][C][C][C][C][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][C][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(8, points)
    }

    @Test
    internal fun `fortress 5`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][M][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][M][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(0, points)
    }

    @Test
    internal fun `fortress 6`() {
        val points = Fortress37.evaluate(
            Board.create(
                """
[C][C][C][ ][ ][ ][ ][ ][ ][ ][ ]
[C][C][ ][M][ ][ ][ ][ ][ ][ ][ ]
[C][ ][ ][ ][ ][ ][ ][ ][M][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[C][C][ ][ ][ ][M][ ][ ][ ][ ][ ]
[C][C][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[C][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][M][ ][ ][ ][ ][ ][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][M][ ][ ][ ]
[ ][ ][ ][ ][ ][ ][ ][ ][ ][ ][ ]
"""
            )
        )
        Assertions.assertEquals(10, points)
    }

}